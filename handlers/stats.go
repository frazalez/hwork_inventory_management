...
func TestingStatGraph(c echo.Context) error {
	graph := model.TestBarChart()
	graph.Render(c.Response().Writer)
	return nil
}
...
func CalculateSalesOfProductInTimespan(c echo.Context) error {
	...
	fmt.Printf("%v \n \n", mostSoldProductData[0])

	chart := model.SalesFromDateBarChart(mostSoldProductData)
	return chart.Render(c.Response())
}
...
func CalculateProductDeltaInTimespan(c echo.Context) error {
	...
	chart := model.ProductDeltaTimespanChart(productSaleDelta)
...
func CalculateProfitDeltaInTimespan(c echo.Context) error {
	...
	chart := model.ProfitDeltaTimespanChart(profitDelta)
...
func CalculateSaleAmountDeltaTimespan(c echo.Context) error {
	...
	chart := model.SalesDeltaTimespanChart(productSaleDelta)
...
func CalculateDeltaMoneySpentProduct(c echo.Context) error {
	...
	chart := model.MoneySpentProductDeltaTimespanChart(moneySpentDelta)
	return chart.Render(c.Response())
}
...
func CalculateMoneySpentDeltaTimespan(c echo.Context) error {
	...
	chart := model.MoneySpentDeltaTimespanChart(moneySpentDelta)
